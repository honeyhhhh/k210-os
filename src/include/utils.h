...
#define kendryte_write_dword(dest, src) \
    (*KENDRYTE_CAST(volatile uint64_t_t *, (dest)) = (src))
 ...
 */
#define kendryte_read_dword(src) (*KENDRYTE_CAST(volatile uint64_t_t *, (src)))
 ...
 * @param[in]   value       The value to set
 */
void set_gpio_bit(volatile uint32_t *bits, uint64_t idx, uint32_t value);
