...
void spi_init(spi_device_num_t spi_num, spi_work_mode_t work_mode, spi_frame_format_t frame_format,
              uint64_t data_bit_length, uint32_t endian)
{
...
static spi_transfer_width_t spi_get_frame_size(uint64_t data_bit_length)
{
...
void spi_send_data_normal(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uint8_t *tx_buff, uint64_t tx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);

    uint64_t index, fifo_len;
...
void spi_send_data_standard(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uint8_t *cmd_buff,
                            uint64_t cmd_len, const uint8_t *tx_buff, uint64_t tx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
...
void spi_receive_data_standard(spi_device_num_t spi_num, spi_chip_select_t chip_select, const uint8_t *cmd_buff,
                               uint64_t cmd_len, uint8_t *rx_buff, uint64_t rx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    uint64_t index, fifo_len;
    if(cmd_len == 0)
    ...
    uint32_t i = 0;
    uint64_t v_cmd_len = cmd_len / frame_width;
...
void spi_receive_data_normal_dma(dmac_channel_number_t dma_send_channel_num,
                                 ...
                                 uint64_t cmd_len, void *rx_buff, uint64_t rx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
...
void spi_send_data_normal_dma(dmac_channel_number_t channel_num, spi_device_num_t spi_num,
                              ...
                              const void *tx_buff, uint64_t tx_len, spi_transfer_width_t spi_transfer_width)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    spi_set_tmod(spi_num, SPI_TMOD_TRANS);
    volatile spi_t *spi_handle = spi[spi_num];
    ...
    switch(spi_transfer_width)
    {
        case SPI_TRANS_SHORT:
            ...
            buf = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, (tx_len)*sizeof(uint32_t));
            for(i = 0; i < tx_len; i++)
                                   ...
                                   uint64_t cmd_len, uint8_t *rx_buff, uint64_t rx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            write_cmd = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, cmd_len + rx_len);
            ...
            break;
        case SPI_TRANS_SHORT:
            write_cmd = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, (cmd_len + rx_len) / 2 * sizeof(uint32_t));
            ...
            break;
        default:
            write_cmd = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, (cmd_len + rx_len) * sizeof(uint32_t));
            for(i = 0; i < cmd_len; i++)
...
void spi_send_data_standard_dma(dmac_channel_number_t channel_num, spi_device_num_t spi_num,
                                spi_chip_select_t chip_select,
                                const uint8_t *cmd_buff, uint64_t cmd_len, const uint8_t *tx_buff, uint64_t tx_len)
{
    // configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);
    ...
    {
        case SPI_TRANS_INT:
            buf = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, cmd_len + tx_len);
            for(i = 0; i < cmd_len / 4; i++)
            ...
            break;
        case SPI_TRANS_SHORT:
            buf = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, (cmd_len + tx_len) / 2 * sizeof(uint32_t));
            for(i = 0; i < cmd_len / 2; i++)
            ...
            break;
        default:
            buf = (uint32_t *)buddy_alloc(HEAP_ALLOCATOR, (cmd_len + tx_len) * sizeof(uint32_t));
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len; i++)
